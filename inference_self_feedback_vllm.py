...
def load_vllm_model(model_id: str, gpu_memory_utilization: float = 0.90):
    """
    Reasoning과 Self-Feedback을 모두 수행할 단일 모델을 로드합니다.
    """
...
def run_dynamic_batch_inference_self_feedback(
    ...
    pbar = tqdm(total=len(df), desc="Self-Feedback vLLM Processing")
...
Generate next step (start with `Step {len(state['step_texts']) + 1}:`)"""
            ...
            if is_qwen3_model:
                gen_template_kwargs["enable_thinking"] = False
            gen_prompts.append(tokenizer.apply_chat_template(messages, **gen_template_kwargs))
            gen_indices.append(idx)
            gen_force_answer_modes.append(force_answer_mode)

        # vLLM Batch Run (Generator)
        if gen_prompts:
            ...
            gen_outputs = llm.generate(
                gen_prompts, 
                sampling_params=SamplingParams(temperature=0.0, max_tokens=256, stop_token_ids=stop_tokens),
                use_tqdm=False
            )
...
""".strip()
            ...
            eval_prompts.append(tokenizer.apply_chat_template(messages, **eval_template_kwargs))
        ...
        if eval_prompts:
            eval_sampling_kwargs = {
                ...
                "structured_outputs": StructuredOutputsParams(json=eval_json_schema),
            }

            eval_outputs = llm.generate(
                eval_prompts, 
                SamplingParams(**eval_sampling_kwargs), 
                use_tqdm=False
            )
...
if __name__ == "__main__":
    ...
    generator_model_id = args.model_id.lower()
    is_qwen36_27b = "qwen3.6" in generator_model_id and "27b" in generator_model_id

    if "llama" in generator_model_id and "-8b" in generator_model_id:
        model_name = "llama8b"
    elif "llama" in generator_model_id and "-70b" in generator_model_id:
        model_name = "llama70b"
    elif is_qwen36_27b:
        model_name = "qwen36_27b"
    elif "qwen" in generator_model_id and "-27b" in generator_model_id:
        model_name = "qwen27b"
    elif "qwen" in generator_model_id and "-8b" in generator_model_id:
        model_name = "qwen8b"
    elif "qwen" in generator_model_id and "-4b" in generator_model_id:
        model_name = "qwen4b"
    elif "qwen" in generator_model_id and "-7b" in generator_model_id:
        model_name = "qwen7b"
    elif "qwen" in generator_model_id and "-14b" in generator_model_id:
        model_name = "qwen14b"
    elif "gemma" in generator_model_id and "-12b" in generator_model_id:
        model_name = "gemma12b"
    elif "oss" in generator_model_id:
        model_name = "oss20b"
    else:
        model_name = "unknown"
    ...
    if args.sample_limit > 0:
        limit_n = min(args.sample_limit, len(df))
        print(f"Keeping first {limit_n} rows after sampling; sample_limit={args.sample_limit}.")
        df = df.head(limit_n)
