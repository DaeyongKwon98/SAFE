...
Your Goal: Replace a single, correct reasoning step with an 'Unsupported' error.

Error Definition: 'Unsupported'
An 'Unsupported' error is an (Attribution) step that makes a factual claim using information that **cannot** be found in any of the `Retrieved Passages`. The reasoning step hallucinates a new, false piece of information and presents it as fact.
- The new step's content (the "fact") MUST be **completely absent** from all `Retrieved Passages`.
- The new step MUST NOT directly contradict the information in the passages. It should be additional or supplementary information that sounds plausible but is simply not in the source text.
- The new step MUST still look like a plausible attribution ("According to Passage X, ...").
- The new step MUST maintain the original step's `(Attribution)` label.
...
Output:
Step 2: According to Passage 2, Kane Brown (from Step 1) released his first EP \"Closer\" under the Sony Music Nashville label. (Attribution)
...
Output: 
Step 2: According to Passage 2, the Bengali political film 'Kangal Malsat' was censored by the Central Board of Film Certification due to its controversial depiction of Stalin. (Attribution)
...
Output: 
Step 1: According to Passage 1, David Nixon is an American film director who was born in Chicago, Illinois. (Attribution)
...
def main(args):
    ...
    output_filepath = f"/workspace/daeyong/ideal_steps/{args.dataset}_unsupported_rewriting.json"
    ...
    # =======================================================
    for item in tqdm(data, desc="Injecting 'Unsupported' errors"):
        try:
...
""".strip()
                ...
                # (Note: Unsupported definition says it must maintain Attribution label)
                ...
                new_item['error_type'] = 'Unsupported'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inject 'Unsupported' errors into reasoning steps.")
