...
def main(args):
    ...
    # data_path = f"{base_dir}/fourth_finetuning_data/final_sft_data_before_triple.json"
    
    # Unique Passage들에 대한 Triple 결과를 저장할 경로
    output_path = f"{base_dir}/knowledge_graphs/{args.dataset}_triples.json"
    # output_path = f"{base_dir}/fourth_finetuning_data/final_sft_data_triples.json"
    
    # Mapping 정보가 포함된 DF 저장 경로
    df_output_path = f"{base_dir}/benchmarks/{args.dataset}_indexed.csv" 
    # df_output_path = f"{base_dir}/fourth_finetuning_data/final_sft_data_indexed.csv"
