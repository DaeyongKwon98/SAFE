...
def run_dynamic_batch_inference(
    ...
    def create_new_state(row):
        ...
        return {
            ...
            "passages": passages,
            "answer_list_norm": parse_answer_list(row.get("answer_list")),
            "step_texts": [],
            "feedback_list": [],
            "logs": {
...
""".strip()
        ...
        gen_chat_template_kwargs = None
        if is_qwen3_generator:
            gen_chat_template_kwargs = {
                "enable_thinking": qwen3_thinking_mode == "on"
            }

        gen_outputs = gen_llm.chat(
            gen_messages_batch,
            gen_sampling_params,
            use_tqdm=False,
            chat_template_kwargs=gen_chat_template_kwargs,
        )
...
""".strip()
        ...
        if eval_messages_batch:
            eval_chat_template_kwargs = None
            ...
            eval_sampling_params = SamplingParams(
                ...
                stop_token_ids=build_stop_token_ids(eval_tokenizer),
            )
            eval_outputs = eval_llm.chat(
                ...
                use_tqdm=False,
                chat_template_kwargs=eval_chat_template_kwargs,
            )
        ...
        for state in active_states:
            ...
            if state["finished"]:
                ...
                res_obj = {
                    ...
                    "answer_list_norm": state.get("answer_list_norm", []),
                    "skipped": is_skipped,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help="Maximum context length used when loading the generator vLLM engine.",
    )
    parser.add_argument(
        "--evaluator_max_model_len",
        type=int,
        default=8000,
        help="Maximum context length used when loading the evaluator vLLM engine.",
    )
    parser.add_argument(
        "--qwen3_thinking_mode",
        type=str,
    ...
    parser.add_argument(
        "--benchmark_variant",
        type=str,
        default="dev_kg_correct",
        help="Benchmark filename variant loaded as benchmarks/{dataset}_{variant}.csv.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Explicit output directory. Defaults to the existing feedback-model based path.",
    )
    parser.add_argument(
        "--track_cache_stats",
        action="store_true",
    ...
    benchmark_path = f"/workspace/daeyong/benchmarks/{args.dataset}_{args.benchmark_variant}.csv"
    print(f"📚 Benchmark File: {benchmark_path}")
    df = pd.read_csv(benchmark_path).sample(n=200, random_state=42)[:100] # 임시로 100개만!
