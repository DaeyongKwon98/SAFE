...
class GeminiSelfFeedbackClient:
    """Single Gemini model used for both generation and self-evaluation."""
    ...
    def _build_payload(
        ...
        if response_format is not None:
            mime_type = response_format.get("responseMimeType")
            json_schema = response_format.get("responseJsonSchema")
            if mime_type:
    ...
    def _chat_completion(
        ...
        raise RuntimeError(f"Gemini call failed after retries: {last_error}") from last_error
...
def load_gemini_model(
    ...
    """
    Reasoning과 Self-Feedback을 모두 수행할 단일 Gemini API 모델을 초기화합니다.
    """
    print(f"Loading Single Model (Gemini API)... Model: '{model_id}'")
    client = GeminiSelfFeedbackClient(
        model=model_id,
...
def run_dynamic_batch_inference_self_feedback(
    ...
    pbar = tqdm(total=len(df), desc="Self-Feedback Gemini API Processing")
...
Generate next step (start with `Step {len(state['step_texts']) + 1}:`)"""
        ...
        # Gemini API Batch Run (Generator)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gemini_eval_max_completion_tokens", type=int, default=256)
    parser.add_argument("--gemini_temperature", type=float, default=None)
    parser.add_argument("--gemini_eval_temperature", type=float, default=None)
    parser.add_argument(
        "--gemini_eval_response_format",
        type=str,
    ...
    if args.sample_size > 0:
        ...
        df = df.sample(n=sample_n, random_state=args.sample_seed)[:100] # 임시로 100개로 설정
    ...
    if len(df) > 0:
        ...
        run_dynamic_batch_inference_self_feedback(
            ...
            eval_max_completion_tokens=args.gemini_eval_max_completion_tokens,
            temperature=args.gemini_temperature,
            eval_temperature=args.gemini_eval_temperature,
            eval_response_format=args.gemini_eval_response_format,
        )
    else:
